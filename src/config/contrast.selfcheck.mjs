...
const PAIRS = [
  ...
  /* GRADING THE CORE AGAIN, 2026-08-24 (SECOND TIME TODAY). These three briefly measured the
  ...
  /* 🔴 THE HOVER-ONLY BUTTON AT REST. 22 buttons carry text-[var(--gold-ink)] permanently but
     get their plate only on hover, so at rest bone ink sits on --raised at 1,11:1 - invisible,
     and on a phone there is no hover to rescue it. A CSS rule in theme.css gives those buttons
     --accent-ink until they are hovered; this pair is what makes that rule non-optional. It is
     graded on --raised because that is the lightest ground any of them sits on. */
  ['hover-only button at rest',     'accent-ink',    'raised',       4.5],
